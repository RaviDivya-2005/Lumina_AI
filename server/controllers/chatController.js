...
const sendMessage = async (req, res) => {
  try {
    ...
    // Handle local fallback IDs — chat was never persisted to Supabase
    const isLocalChat = chatId && chatId.startsWith('local-');
    let chat = null;
    if (!isLocalChat) {
      const chats = await supabaseRequest('GET', `/rest/v1/chat_histories?id=eq.${chatId}&select=*`);
      chat = Array.isArray(chats) ? chats[0] : chats;
    }
    // If chat not found (local fallback or missing), still process the AI message
    if (!chat) {
...
const sendMessageStream = async (req, res) => {
  try {
    ...
    // If chat not found (local fallback or DB error), use an in-memory placeholder
    if (!chat) {
      chat = { id: chatId, title: 'New Chat', messages: [] };
    }
    ...
    // Save user message immediately so it persists even if stream fails (skip for local chats)
    ...
    const groqReq = callGroqStream(
      ...
      () => {
        ...
        // Only save to DB if this is a real (non-local) chat
        if (!isLocalChat) {
      ...
      (err) => {
        ...
        callGroqAPI(streamMessages)
          .then(groqResult => {
            ...
            if (!isLocalChat) {
              return supabaseRequest('GET', `/rest/v1/chat_histories?id=eq.${chatId}&select=*`)
                ...
                .then(sendFallbackResponse)
                .catch(sendFallbackResponse);
            } else {
          ...
          .catch(fallbackErr => {
            ...
            const fallback = 'I apologize, but I am currently experiencing a temporary issue. Please try asking your question again.';
