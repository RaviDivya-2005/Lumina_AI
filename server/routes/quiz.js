        ...
        if (userRes.ok) {
          ...
          if (email) {
            ...
            if (Array.isArray(studentsData) && studentsData.length > 0) {
              ...
              // Not in students table — check profiles (Admin/SuperAdmin)
              const profileData = await supabaseRequest('GET', `/rest/v1/profiles?select=id,workspace_id&email=eq.${encodeURIComponent(email)}`);
              let wsId = workspaceId;
              if (Array.isArray(profileData) && profileData.length > 0) {
                wsId = wsId || profileData[0].workspace_id;
              }
              // Ensure we have a workspace
              if (!wsId) {
                const wsData = await supabaseRequest('GET', '/rest/v1/workspaces?select=id&limit=1');
                wsId = Array.isArray(wsData) && wsData.length > 0 ? wsData[0].id : null;
              }
              // Auto-create a student record for this user
              if (wsId && email) {
                const safeName = (email.split('@')[0] || 'User').replace(/[^a-zA-Z0-9 ]/g, '');
                try {
                  const newStudent = await supabaseRequest('POST', '/rest/v1/students', {
                    student_id: `AUTO-${Date.now()}`,
                    name: safeName,
                    email: email,
                    workspace_id: wsId,
                  });
