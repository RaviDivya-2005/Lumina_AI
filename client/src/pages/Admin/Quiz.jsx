    ...
    <AdminLayout>
      <div className="space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage categories, questions, and view results</p>
        </motion.div>
          ...
          <div className="flex border-b border-slate-200 dark:border-white/10">
            {tabs.map(tab => (
              ...
              >
                ...
                {tab.label}
                {activeTab === tab.key && (
                  <motion.div layoutId="quiz-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-655 dark:bg-white" />
                )}
              </button>
            ))}
          </div>
...
function CategoriesTab() {
  ...
  return (
    <>
      {error && (
        <div className="rounded-2xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/5 backdrop-blur-xl p-4 text-center mb-4">
          <p className="text-red-600 dark:text-red-400 text-sm mb-3">{error}</p>
          <button onClick={fetchCategories} className="px-4 py-2 rounded-xl bg-slate-100 text-gray-700 hover:bg-slate-200 dark:bg-white/10 dark:text-gray-300 dark:hover:bg-white/20 transition-colors">Retry</button>
        </div>
      )}
      ...
      <Card padding="none" className="overflow-hidden">
        {categories.length === 0 ? (
          <div className="p-12 text-center">
            <Layers className="w-12 h-12 text-gray-400 dark:text-gray-600 mx-auto mb-3" />
          ...
          <div className="divide-y divide-slate-100 dark:divide-white/5">
            {categories.map((cat, idx) => (
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex items-center justify-between px-6 py-4 hover:bg-slate-50/50 dark:hover:bg-white/[0.03] transition-colors"
              >
                <div className="flex items-center gap-3">
                  <div className="rounded-lg bg-indigo-50 dark:bg-white/10 p-2">
                    <Layers className="w-4 h-4 text-indigo-600 dark:text-white" />
                  </div>
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => openEdit(cat)} className="p-1.5 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-slate-100 dark:text-gray-500 dark:hover:text-white dark:hover:bg-white/10 transition-colors"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => setDeleteConfirm(cat)} className="p-1.5 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-slate-100 dark:text-gray-500 dark:hover:text-white dark:hover:bg-white/10 transition-colors"><Trash2 className="w-4 h-4" /></button>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <Modal isOpen={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Delete Category" size="sm">
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-700 dark:text-gray-300">Delete <span className="font-semibold text-gray-900 dark:text-white">{deleteConfirm?.name}</span>?</p>
          <div className="flex gap-3">
  ...
  return (
    <>
      {error && (
        <div className="rounded-2xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/5 backdrop-blur-xl p-4 text-center mb-4">
          <p className="text-red-600 dark:text-red-400 text-sm mb-3">{error}</p>
          <button onClick={fetchQuestions} className="px-4 py-2 rounded-xl bg-slate-100 text-gray-700 hover:bg-slate-200 dark:bg-white/10 dark:text-gray-300 dark:hover:bg-white/20 transition-colors">Retry</button>
        </div>
      )}
      <div className="flex items-center justify-between mb-4">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-white/10">
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400">Question</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400">Topic</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400">Difficulty</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {questions.map((q, idx) => (
                  <motion.tr
                    ...
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="hover:bg-slate-50/50 dark:hover:bg-white/[0.03] transition-colors"
                  >
                    <td className="px-4 py-3 text-gray-700 dark:text-gray-200 max-w-xs truncate">{q.questionText}</td>
                    <td className="px-4 py-3">
                      <Badge variant="default">{categories.find(c => (c.id === (q.categoryId || q.category)))?.name || q.categoryName || 'Uncategorized'}</Badge>
                    </td>
                    <td className="px-4 py-3">
                      {q.topic ? <Badge variant="default">{q.topic}</Badge> : <span className="text-gray-400 dark:text-gray-500 text-xs">—</span>}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEdit(q)} className="p-1.5 rounded-lg text-gray-500 hover:text-gray-900 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => setDeleteConfirm(q)} className="p-1.5 rounded-lg text-gray-500 hover:text-red-600 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </motion.tr>
                ))}
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/5 px-4 py-2.5 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:border-indigo-500 dark:focus:border-white/30 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-white/10 focus:outline-none transition-all resize-none"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {form.options.map((opt, i) => (
              <div key={i}>
                <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Option {i + 1}</label>
                <input
                  required
                  value={opt}
                  onChange={e => {
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/5 px-3 py-2 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:border-indigo-500 dark:focus:border-white/30 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-white/10 focus:outline-none transition-all text-sm"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/5 px-4 py-2.5 text-gray-900 dark:text-gray-100 focus:border-indigo-500 dark:focus:border-white/30 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-white/10 focus:outline-none transition-all text-sm"
              >
                {form.options.map((_, i) => (
                  <option key={i} value={i} className="text-gray-900">Option {i + 1}</option>
                ))}
              </select>
            </div>
            <div>
              ...
              >
                ...
                {categories.map(c => (
                  <option key={c._id || c.id} value={c._id || c.id} className="text-gray-900">{c.name}</option>
                ))}
              </select>
            </div>
            <div>
              ...
              >
                {difficulties.map(d => (
                  <option key={d} value={d} className="text-gray-900">{d.charAt(0).toUpperCase() + d.slice(1)}</option>
                ))}
              </select>
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Explanation (optional)</label>
            <textarea
      ...
      <Modal isOpen={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Delete Question" size="sm">
        <div className="text-center space-y-4">
          <AlertCircle className="w-12 h-12 text-red-600 dark:text-red-400 mx-auto" />
          <p className="text-gray-700 dark:text-gray-300 font-medium">Delete this question?</p>
          <div className="flex gap-3">
            <Button variant="danger" onClick={() => handleDelete(deleteConfirm)} className="flex-1">Delete</Button>
            <Button variant="ghost" onClick={() => setDeleteConfirm(null)}>Cancel</Button>
          </div>
        ...
        {aiResult ? (
          <div className="text-center space-y-4 py-4">
            {aiResult.error ? (
              <>
                <AlertCircle className="w-12 h-12 text-red-600 dark:text-red-400 mx-auto" />
                <p className="text-red-600 dark:text-red-400 text-sm">{aiResult.error}</p>
              </>
            ) : (
              <>
                <Check className="w-12 h-12 text-indigo-600 dark:text-white mx-auto" />
                <p className="text-gray-900 dark:text-gray-200 font-medium font-bold">Questions Generated!</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">{aiResult.generated?.length || 0} questions created ({aiResult.saved || 0} saved)</p>
              </>
            )}
          ...
          <form onSubmit={handleAIGenerate} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Topic / Focus (optional)</label>
              <input
                value={aiForm.topic}
...
function ResultsTab() {
  ...
  return (
    <>
      {error && (
        <div className="rounded-2xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/5 backdrop-blur-xl p-4 text-center mb-4">
          <p className="text-red-655 dark:text-red-400 text-sm mb-3">{error}</p>
          <button onClick={fetchResults} className="px-4 py-2 rounded-xl bg-slate-100 text-gray-700 hover:bg-slate-200 dark:bg-white/10 dark:text-gray-300 dark:hover:bg-white/20 transition-colors">Retry</button>
        </div>
      )}
      <Card padding="none" className="overflow-hidden">
        {results.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-500 text-sm">No quiz results yet</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-white/10">
                  {['Student', 'Category', 'Score', 'Percentage', 'Date'].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {results.map((r, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{r.score}/{r.totalQuestions || r.total}</td>
                    <td className="px-4 py-3">
                      <Badge variant="default">
                        {Math.round(r.percentage)}%
                      </Badge>
                    </td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-500 text-xs">{formatDate(r.createdAt || r.date)}</td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
