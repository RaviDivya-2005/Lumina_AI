    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Quiz Practice</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Test your knowledge across various subjects</p>
          </div>
          {view === 'categories' && (
        ...
        {error && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 backdrop-blur-xl p-4 text-center"
          >
            <p className="text-red-600 dark:text-red-400 text-sm mb-3">{error}</p>
            <button onClick={() => setError(null)} className="text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 text-sm font-medium">Dismiss</button>
          </motion.div>
        )}
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {loading ? (
                  ...
                  <div className="col-span-full text-center py-16">
                    ...
                    <h3 className="text-lg font-medium text-gray-600 dark:text-gray-300">No categories available</h3>
                  </div>
                ) : (
                      ...
                      >
                        <Card
                          ...
                          className={`cursor-pointer transition-all duration-200 ${
                            isSelected
                              ? 'ring-2 ring-indigo-600 border-indigo-600/50 dark:ring-white dark:border-white/50'
                              : ''
                          }`}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
                <Card>
                  <div className="flex items-center gap-2 mb-4">
                    <BarChart3 className="w-5 h-5 text-indigo-600 dark:text-white" />
                ...
                <Card>
                  <div className="flex items-center gap-2 mb-4">
                    <Trophy className="w-5 h-5 text-yellow-600 dark:text-white" />
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="rounded-xl bg-indigo-50 dark:bg-white/10 p-2.5">
                    <Brain className="w-5 h-5 text-indigo-600 dark:text-white" />
                  </div>
                  <div>
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{selectedCategory?.name} Quiz</h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Question {currentQuestion + 1} of {qs.length}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 dark:bg-white/5 dark:border-white/10">
                  <Clock className="w-4 h-4 text-indigo-600 dark:text-white" />
                  <span className={`text-sm font-mono font-bold ${timer < 60 ? 'text-red-500' : 'text-gray-700 dark:text-gray-200'}`}>
                    {formatTime(timer)}
                  </span>
                </div>
              </div>
              ...
              {quiz.topics && quiz.topics.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap">
                  <BookOpen className="w-3.5 h-3.5 text-indigo-600 dark:text-white" />
                  {quiz.topics.map((t) => (
                    ...
                    return (
                      <motion.button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left text-sm transition-all duration-200 border ${
                          isSelected
                            ? 'border-indigo-600 bg-indigo-50 text-indigo-600 dark:border-white dark:bg-white/20 dark:text-white'
                            : selectedAnswer !== null
                            ? 'border-slate-100 text-gray-400 opacity-50 dark:border-white/5'
                            : 'border-slate-200 text-gray-700 hover:border-indigo-600 hover:bg-slate-50 dark:border-white/10 dark:text-gray-300 dark:hover:border-white/20 dark:hover:bg-white/5'
                        }`}
                      >
                        <span className={`flex-shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold ${
                          isSelected
                            ? 'bg-indigo-600 text-white dark:bg-white dark:text-black'
                            : 'bg-slate-100 text-gray-500 dark:bg-white/5 dark:text-gray-400'
                        }`}>
                          {labels[idx]}
                        </span>
                        {option}
          ...
          {view === 'in-progress' && (!quiz || !currentQ) && !starting && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">No questions available for this category yet. Generate some with AI from Quiz Management.</p>
              ...
              <Card className="text-center">
                <div className="relative">
                  <motion.div
                    ...
                    className={`w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg bg-indigo-50 dark:bg-white`}
                  >
                    {result.percentage >= 70 ? (
                      <Trophy className="w-9 h-9 text-indigo-600 dark:text-black" />
                    ) : (
                      <Brain className="w-9 h-9 text-indigo-600 dark:text-black" />
                    )}
                  </motion.div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">Quiz Complete!</h2>
                  <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">{selectedCategory?.name}</p>
                  ...
                  <div className="flex items-center justify-center gap-4">
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-50 text-indigo-600 dark:bg-white/10 dark:text-white text-sm font-semibold">
                      <CheckCircle2 className="w-4 h-4" />
                      {result.correctCount ?? result.score} Correct
                    </div>
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-50 text-red-600 dark:bg-white/10 dark:text-white text-sm font-semibold">
                    ...
                    {result.answers.map((ans, idx) => (
                      <div
                        ...
                        className={`p-4 rounded-xl border ${
                          ans.isCorrect
                            ? 'border-emerald-200 bg-emerald-50/30 dark:border-white/20 dark:bg-white/5'
                            : 'border-slate-200 bg-slate-50/50 dark:border-white/10 dark:bg-white/5'
                        }`}
                      >
                        <div className="flex items-start gap-3">
                          <div className={`flex-shrink-0 w-6 h-6 rounded-lg flex items-center justify-center ${
                            ans.isCorrect ? 'bg-emerald-100 text-emerald-700 dark:bg-white/20 dark:text-white' : 'bg-red-100 text-red-700 dark:bg-white/10 dark:text-white'
                          }`}>
                            {ans.isCorrect ? (
                              <CheckCircle2 className="w-4 h-4" />
                            ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">{ans.questionText || `Question ${idx + 1}`}</p>
                            <div className="text-xs space-y-1">
                              <p className="text-gray-600 dark:text-gray-400">
                                Your answer: <span className={ans.isCorrect ? 'text-emerald-700 dark:text-white' : 'text-red-600 dark:text-red-400 font-semibold'}>{ans.selectedAnswer}</span>
                              </p>
                              {!ans.isCorrect && (
                                <p className="text-gray-600 dark:text-gray-400">
                                  Correct answer: <span className="text-indigo-600 dark:text-white font-medium">{ans.correctAnswer}</span>
                                </p>
                              )}
                            </div>
                            {ans.explanation && (
                              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 italic">{ans.explanation}</p>
                            )}
                          </div>
                        </div>
                      </div>
